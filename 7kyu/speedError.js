...
var speedError = function (est, act, readings) {
  ...
  const speed = (h) => {
    const x1 = Math.sqrt(Math.abs(d1 * d1 - h * h));
    const x2 = Math.sqrt(Math.abs(d2 * d2 - h * h));

    return +((Math.abs(x1 - x2) / Math.abs(t1 - t2)) * 3.6).toFixed(1);
  };
