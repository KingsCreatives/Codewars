...
function getNumberType(number) {
  ...
  const mantissaZero = mantissaHigh === 0 && low === 0;

  const prefix = sign ? 'NEGATIVE_' : 'POSITIVE_';

  if (exponent === 0x7ff) {
    if (mantissaZero) {
      return FloatType[prefix + 'INFINITY'];
    }

    if (mantissaHigh & 0x80000) {
      return FloatType[prefix + 'QUIET_NAN'];
    }

    return FloatType[prefix + 'SIGNALING_NAN'];
  }

  if (exponent === 0) {
    if (mantissaZero) {
      return FloatType[prefix + 'ZERO'];
    }

    return FloatType[prefix + 'DENORMALIZED'];
  }

  return FloatType[prefix + 'NORMALIZED'];
}