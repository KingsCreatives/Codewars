...
function GuessWho(character) {
  ...
  this.possible = this.characteristic.slice();
}

GuessWho.prototype.guess = function (guess) {
  this.turns++;

  if (this.characteristic.some(([name]) => name === guess)) {
    if (guess === this.character) {
      return [`Correct! in ${this.turns} turns`];
    }

    this.possible = this.possible.filter(([name]) => name !== guess);
    return this.possible.map(([name]) => name);
  }

  const target = this.characteristic.find(([name]) => name === this.character);
  const hasTrait = target[1].includes(guess);

  this.possible = this.possible.filter(([_, traits]) =>
    hasTrait ? traits.includes(guess) : !traits.includes(guess),
  );
